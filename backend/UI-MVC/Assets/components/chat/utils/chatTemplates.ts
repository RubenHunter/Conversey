...
export function workspaceAvatarHTML(workspaceBadge: string, logoUrl?: string): string {
    if (logoUrl) {
        return `<img src="${logoUrl}" alt="" class="chat-avatar-img" />`
    }
...
export function avatarHTML(workspaceBadge: string, inIdeasPhase: boolean, logoUrl?: string): string {
    if (inIdeasPhase) {
        return `${AI_AVATAR}<span class="chat-avatar-badge">AI</span>`
    }
    return workspaceAvatarHTML(workspaceBadge, logoUrl)
}
