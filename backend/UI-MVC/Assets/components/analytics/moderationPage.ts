...
class ModerationPage {
    ...
    private bindAcceptButtons() {
        document.querySelectorAll('.moderate-btn-accept').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const allBtns = card.querySelectorAll('button.moderate-btn-accept, button.moderate-btn-deny');

                if (!confirm(`Approve this ${type} and make it visible to all users?`)) return;
    ...
    private collectReason(): string | null {
        ...
        this.denyFlagCheckboxes.forEach(cb => {
            if (cb.checked) {
                const label = FLAG_LABELS[cb.dataset.flag!] ?? cb.dataset.flag!;
                parts.push(label);
            }
        });
    ...
    private resetDenyModal() {
        this.denyFlagCheckboxes.forEach(cb => { cb.checked = false; });
        if (this.denyFreeText) this.denyFreeText.value = '';
    }

    private openDenyModal(card: HTMLElement) {
        this.resetDenyModal();
        ...
        const typeLabel = (card.dataset.type === 'idea' ? 'Idea' : 'Comment') + ' #' + card.dataset.id;
    ...
    private closeDenyModal() {
        this.denyModal?.classList.add('hidden');
        this.denyModal?.classList.remove('flex');
    }

    private reenableCardButtons() {
        if (!this.pendingDenyCard) return;
        const allBtns = this.pendingDenyCard.querySelectorAll('button.moderate-btn-accept, button.moderate-btn-deny');
        allBtns.forEach(b => (b as HTMLButtonElement).disabled = false);
    }

    private executeModeration(type: string, id: number, action: string, reason: string | null) {
        fetch('/api/admin/analytics/moderate', {
            method: 'POST',
            ...
            .catch(() => {
                this.showCardError('Network error');
                this.reenableCardButtons();
            });
    }

    private showCardError(message: string) {
        if (!this.pendingDenyCard) return;
        const resultEl = this.pendingDenyCard.querySelector('.moderation-result') as HTMLElement;
        if (resultEl) {
    ...
    private collapseCard(card: HTMLElement, action: string) {
        const resultEl = card.querySelector('.moderation-result') as HTMLElement;
        if (resultEl) {
            resultEl.textContent = action === 'accept' ? 'Approved' : 'Rejected';
            resultEl.classList.remove('hidden');
            resultEl.className =
    ...
    private updateCountAndEmptyState() {
        ...
        if (countEl) {
            countEl.textContent = remaining + ' item' + (remaining !== 1 ? 's' : '') + ' pending review';
        }
        ...
        if (remaining === 0) {
            ...
            if (spaceDiv) {
                spaceDiv.outerHTML =
                    ...
                    '<p class="text-sm text-secondary">No ideas or comments are currently pending review.</p>' +
                    '</div>';
            }
        }
    }
